...
// --- MULTI-LANG LUG'ATI ---
const i18n = {
    uz: { welcome: "tizimiga xush kelibsiz!", sub_req: "Botdan foydalanish uchun kanallarga a'zo bo'ling yoki so'rov yuboring:", check: "✅ Tekshirish", signal: "🚀 Signal olish", vip_signal: "⚡️ Signal olish (VIP)", apps: "📱 Ilovalar", ref: "👥 Yo'llanma silka", guide: "📖 Bot bilan tanishish", terms: "📜 Signal olish shartlari", platform: "🎯 <b>Platformani tanlang:</b>", input_id: "🆔 ID yuboring (faqat raqam):", wait_admin: "⏳ Qabul qilindi, admin tasdiqlashini kuting.", back: "🔙 Orqaga", no_sub: "❌ Obuna yoki so'rov topilmadi!", verified: "✅ Tasdiqlandi!", ref_text: (count, task, link) => `👥 <b>Referal tizimi</b>\n\n📊 Odamlar: <b>${count}</b> ta\n🎯 Vazifa: <b>${task}</b> ta\n\n🔗 Havolangiz:\n<code>${link}</code>`, terms_text: "<b>📜 SIGNAL OLISH SHARTLARI:</b>\n\n1. Ro'yxatdan o'tishda <b>RICHI28</b> promokodini ishlating.\n2. Balansni kamida 60,000 so'mga to'ldiring.\n3. O'yin ID raqamini botga yuboring.\n\n⚠️ Diqqat: Shartlar bajarilmasa, Hack tizimi xato ishlashi mumkin!", guide_text: "<b>📖 BOTDAN FOYDALANISH QO'LLANMASI:</b>\n\n1. Avval kanallarga obuna bo'ling.\n2. 'Signal olish' tugmasini bosing.\n3. O'zingiz o'ynayotgan platformani tanlang va ID yuboring.\n4. Admin tasdiqlagach, VIP tugmasi orqali Web App'ga kiring.\n5. O'yinni tanlang va 'GET SIGNAL' bosing!" },
    ru: { welcome: "Добро пожаловать в систему!", sub_req: "Для использования бота подпишитесь на каналы или отправьте запрос:", check: "✅ Проверить", signal: "🚀 Получить сигнал", vip_signal: "⚡️ Получить сигнал (VIP)", apps: "📱 Приложения", ref: "👥 Рефералка", guide: "📖 Инструкция", terms: "📜 Условия получения", platform: "🎯 <b>Выберите платформу:</b>", input_id: "🆔 Отправьте ID (только цифры):", wait_admin: "⏳ Принято, ожидайте подтверждения.", back: "🔙 Назад", no_sub: "❌ Подписка не найдена!", verified: "✅ Подтверждено!", ref_text: (count, task, link) => `👥 <b>Реферальная система</b>\n\n📊 Людей: <b>${count}</b>\n🎯 Задание: <b>${task}</b>\n\n🔗 Ссылка:\n<code>${link}</code>`, terms_text: "<b>📜 УСЛОВИЯ ПОЛУЧЕНИЯ СИГНАЛА:</b>\n\n1. Используйте промокод <b>RICHI28</b> при регистрации.\n2. Пополните баланс минимум на 500 рублей.\n3. Отправьте ваш ID боту.", guide_text: "<b>📖 ИНСТРУКЦИЯ:</b>\n\n1. Подпишитесь на каналы.\n2. Нажмите 'Получить сигнал'.\n3. Выберите платформу и отправьте ID.\n4. После одобрения войдите в Web App.\n5. Выберите игру и нажмите 'GET SIGNAL'!" },
    en: { welcome: "Welcome to the system!", sub_req: "Subscribe to the channels to use the bot:", check: "✅ Check", signal: "🚀 Get Signal", vip_signal: "⚡️ Get Signal (VIP)", apps: "📱 Apps", ref: "👥 Referral link", guide: "📖 How to use", terms: "📜 Terms & Conditions", platform: "🎯 <b>Select platform:</b>", input_id: "🆔 Send ID (numbers only):", wait_admin: "⏳ Received, wait for approval.", back: "🔙 Back", no_sub: "❌ Subscription not found!", verified: "✅ Verified!", ref_text: (count, task, link) => `👥 <b>Referral System</b>\n\n📊 People: <b>${count}</b>\n🎯 Task: <b>${task}</b>\n\n🔗 Link:\n<code>${link}</code>`, terms_text: "<b>📜 TERMS FOR SIGNALS:</b>\n\n1. Use promo code <b>RICHI28</b> during registration.\n2. Top up balance (min. $5).\n3. Send your ID to the bot.", guide_text: "<b>📖 USER GUIDE:</b>\n\n1. Subscribe to channels.\n2. Click 'Get Signal'.\n3. Choose platform and send ID.\n4. After approval, enter Web App.\n5. Select game and click 'GET SIGNAL'!" }
};
...
// --- QATTIQ REAL-TIME NAZORAT (VIP RESET BILAN) ---
...
bot.start(async (ctx) => {
    ...
    if (user.joinedAt.getTime() > (Date.now() - 15000) && refId && refId !== id) {
...
bot.action('a_bc_menu', (ctx) => {
    ctx.editMessageText("Reklama filtri:", Markup.inlineKeyboard([
        [Markup.button.callback("🌍 Hammaga", "bc_all"), Markup.button.callback("🇺🇿 O'zbekcha", "bc_uz")],
...
bot.on(['text', 'photo', 'video', 'animation', 'document'], async (ctx) => {
    ...
    if (step === 'input_id' && ctx.message.text) {
        if (!(await canAccess(ctx))) return ctx.reply("Kanallarga obuna bo'ling!");
        const u = await User.findOne({ userId: uid });
        if (!/^\d+$/.test(ctx.message.text)) return ctx.reply("Raqam yuboring!");
        await User.findOneAndUpdate({ userId: uid }, { gameId: ctx.message.text, bookmaker: ctx.session.selectedApp });
...
// 6. ACTION HANDLERS
bot.action('show_terms', async (ctx) => { if (!(await canAccess(ctx))) return; const u = await User.findOne({ userId: ctx.from.id }); ctx.editMessageText(i18n[u.lang].terms_text, { parse_mode: 'HTML', ...Markup.inlineKeyboard([[Markup.button.callback('🔙', 'back_home')]]) }); });
bot.action('show_guide', async (ctx) => { if (!(await canAccess(ctx))) return; const u = await User.findOne({ userId: ctx.from.id }); ctx.editMessageText(i18n[u.lang].guide_text, { parse_mode: 'HTML', ...Markup.inlineKeyboard([[Markup.button.callback('🔙', 'back_home')]]) }); });
bot.action('ref_menu', async (ctx) => { if (!(await canAccess(ctx))) return; const u = await User.findOne({ userId: ctx.from.id }); const link = `https://t.me/${ctx.botInfo.username}?start=${u.userId}`; ctx.editMessageText(i18n[u.lang].ref_text(u.referralCount, u.refTask, link), { parse_mode: 'HTML', ...Markup.inlineKeyboard([[Markup.button.callback('🔙', 'back_home')]]) }); });
bot.action('get_signal', async (ctx) => { if (!(await canAccess(ctx))) return; const u = await User.findOne({ userId: ctx.from.id }); const apps = await Config.find({ key: 'app' }); const btns = apps.map(a => [Markup.button.callback(a.name, `select_app_${a.name}`)]); btns.push([Markup.button.callback('🔙', 'back_home')]); ctx.editMessageText(i18n[u.lang].platform, { parse_mode: 'HTML', ...Markup.inlineKeyboard(btns) }); });
bot.action(/^select_app_(.+)$/, async (ctx) => { if (!(await canAccess(ctx))) return; const u = await User.findOne({ userId: ctx.from.id }); ctx.session.selectedApp = ctx.match[1]; ctx.session.step = 'input_id'; ctx.editMessageText(i18n[u.lang].input_id, Markup.inlineKeyboard([[Markup.button.callback('🔙', 'get_signal')]])); });
...
bot.launch().then(() => console.log('🚀 RICHI28 SECURE PRO LIVE'));
