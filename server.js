...
// Config modeli - Unique index muammosi butunlay hal qilindi
...
async function canAccess(ctx) {
    ...
    if (user?.status === 'requested') return true; // Zayavka yuborgan bo'lsa o'tadi
...
bot.action('ref_withdraw', async (ctx) => {
    ...
    if (user.referralCount >= user.refTask) {
        ...
        ctx.answerCbQuery(`✅ Vazifa bajarildi! Keyingi bosqich: ${nextTask} ta odam.`, { show_alert: true });
    } else {
        ctx.answerCbQuery(`❌ Yana ${user.refTask - user.referralCount} ta odam qo'shing!`, { show_alert: true });
    }
});
...
bot.action('add_ch', (ctx) => { ctx.session.step = 'ch_name'; ctx.reply("Nomini yuboring:"); });
...
bot.action('a_app_manage', async (ctx) => {
    ...
    btns.push([Markup.button.callback('🔙 Orqaga', 'admin_main')]);
    ctx.editMessageText("📱 <b>Ilovalar:</b>", Markup.inlineKeyboard(btns));
});

bot.action('add_app', (ctx) => { ctx.session.step = 'app_name'; ctx.reply("Nomini yuboring:"); });
...
bot.action(/^confirm_(\d+)$/, async (ctx) => {
    ...
    bot.telegram.sendMessage(ctx.match[1], "✅ Ruxsat berildi!");
    ctx.editMessageText("Tasdiqlandi.");
});

bot.action(/^reject_(\d+)$/, async (ctx) => {
    bot.telegram.sendMessage(ctx.match[1], "❌ Shartlar bajarilmagan.");
...
bot.action('a_bc', (ctx) => { ctx.session.step = 'bc'; ctx.reply("Reklamani yuboring:"); });
...
bot.on('text', async (ctx, next) => {
    ...
    if (step === 'input_id') {
        ...
        bot.telegram.sendMessage(ADMIN_ID, `🆔 ID: <code>${ctx.message.text}</code>\n👤: ${ctx.from.first_name}`, Markup.inlineKeyboard([
            [Markup.button.callback('✅', `confirm_${ctx.from.id}`), Markup.button.callback('❌', `reject_${ctx.from.id}`)]
        ]));
        return;
    }
